...
export default {
  ...
  methods: {
    ...
    async loadNode(node, resolve) {
      ...
      let { data } = await Category.load({ pId: node.data.cateId });
      resolve(data);
    },
    // 打开编辑Modal
    ...
    openAddModal(node, data) {
      //pid 对应的是他的子分类的pid   那就就得用父的cateid
      this.addForm.pId = data.cateId;
    ...
    openDeleteModal(node, data) {
      this.$confirm("此操作将永久删除该分类, 是否继续?", { type: "warning" })
        .then(async () => {
          let { status, msg } = await Category.remove({
            cateId: data.cateId,
          });
          if (status) {
            node.remove();
